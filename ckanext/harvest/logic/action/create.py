...
from ckanext.harvest.logic import HarvestSourceInactiveError
...
def harvest_source_create(context, data_dict):
    ...
    opt = ['active', 'title', 'description', 'user_id', 'publisher_id',
           'config', 'frequency', 'name']
    for o in opt:
...
def harvest_object_create(context, data_dict):
    ...
    # DGU Hack - for some weird reason we get an error in test_queue2 if we assign a job
    # object here instead of a job_id:
    # InvalidRequestError: Can't attach instance <HarvestJob at 0x5ddc7d0>; another instance with key (<class 'ckanext.harvest.model.HarvestJob'>, (u'da8f7d2b-910f-48a6-8af0-d69eba4405a0',)) is already present in this session.
    # so we hacked the validator to return the job_id and save that.
    obj = HarvestObject(
        guid=data.get('guid'),
